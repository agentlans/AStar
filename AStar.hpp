...
template <class T, class Num, class Hash = std::hash<T>>
class DefaultMap {
public:
	void set(const T& x, const Num& y) {
		map[x] = y;
	}
	Num get(const T& x) {
		if (map.count(x) > 0) {
