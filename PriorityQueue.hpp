...
// min priority queue
template <class Key, class Priority, class Hash = std::hash<Key>>
class PriorityQueue {
public:
  ...
  Priority operator[](const Key &k) { return priority.at(k); }
  // Whether priority queue contains key k
  bool contains(const Key &k) { return (priority.count(k) > 0); }
  // Updates key k to have a new priority p
  void update(const Key &k, const Priority &p) {
    remove(k);
    insert(k, p);
  }
  bool empty() { return priority.empty(); }
  size_t size() { return priority.size(); }
  void clear() {
    priority.clear();
    map.clear();
  }
  ...
  bool check_contents(const std::vector<Key> &keys,
...
private:
  std::unordered_map<Key, Priority, Hash> priority;
