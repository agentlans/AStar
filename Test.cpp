...
public:
  ...
  float distance(int v1, int v2) { return weight[{v1, v2}]; }
  ...
  std::pair<int, int> endpoints() { return {p1, p2}; }
  // Known shortest distance
  float get_shortest_dist() { return shortest_dist; }
...
auto distance_wrapper(Graph &g) {
  return [&](int v1, int v2) { return g.distance(v1, v2); };
}
